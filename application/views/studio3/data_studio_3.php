								...
								<?php
								 ...
								 <tr>
									...
									<td style="text-align: center;">
								 		<div class="btn-group">
											<?php
													if (in_array(119, $_SESSION['menu']) || $user['level_usr']==1) {
														?><?=$button_analog?><?php
													}
												 ?>
									...
									<td style="text-align: center;"><?=$data->jml_ptsl?></td>
									<td style="text-align: center;"><?=($data->target-$data->jml_ptsl)?></td>
								 	...
								 	<td>
								 		<div class="btn-group">
											...
											<?php
											if((in_array(107, $_SESSION['menu']) || $user['level_usr']==1) && $data->jml_peta > 0){
											...
											if (in_array(35, $_SESSION['menu']) || $user['level_usr']==1) {
												?>
								        ...
								        ?><a data-toggle="tooltip" title='input target' href="<?= base_url('studio3/target?target=').$data->kd_full; ?>" class="btn btn-sm btn-info"><span class="fa fa-bullseye"></span></a><?php
								      }
											?>
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$('#cari_filter').on('click',function () {
			...
			window.open('<?=base_url()?>studio3/?filter='+s_filter+'&filterkelurahan='+s_filterkelurahan,'_self',false);
		})
			...
			$('#tabel-body').on('click','#btn-map-online',function () {
				var s_filter = $(this).data('id');
				window.open('<?=base_url()?>studio3/petaonline/'+s_filter,'_self',false);
			});
	})
</script>
