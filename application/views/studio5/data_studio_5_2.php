...
<ul class="nav nav-tabs" id="myTab" role="tablist">
  <?php
      if (in_array(47, $_SESSION['menu']) || $user['level_usr']==1) {
        ?><li class="nav-item" style='background:#fff'>
          <a class="nav-link active" id="home-tab" href="<?php echo base_url()?>studio_5_2/data/?search=<?php echo $idblk;?>" role="tab" aria-controls="home" aria-selected="true">Belum Sertipikat</a>
        </li><?php
      }
      if (in_array(49, $_SESSION['menu']) || $user['level_usr']==1) {
        ?><li class="nav-item">
          <a class="nav-link" id="profile-tab" href="<?php echo base_url()?>studio_5_2/register/?search=<?php echo $idblk;?>" role="tab" aria-controls="profile" aria-selected="false">Sudah Sertipikat</a>
        </li><?php
      }
      if (in_array(50, $_SESSION['menu']) || $user['level_usr']==1) {
        ?><li class="nav-item">
          <a class="nav-link" id="dhkp-tab" href="<?php echo base_url()?>studio_5_2/dhkp/?search=<?php echo $idblk;?>" role="tab" aria-controls="profile" aria-selected="false">Data DHKP</a>
        </li><?php
      }
      if (in_array(53, $_SESSION['menu']) || $user['level_usr']==1) {
        ?><li class="nav-item">
  ...
  ?>
                    ...
                    <button style='margin-top:5px;' type='submit' class='btn btn-primary'>cari</button>
                      ...
                      <td><?=$st['luasfisik_ptsl'];?></td>
											<td>
												<div class="btn-group">
                          <?php
                          if (in_array(48, $_SESSION['menu']) || $user['level_usr']==1) {
                            ?><button data-toggle="tooltip" title="input luas" class="btn btn-sm btn-primary btn-open-luas" id="" data-id="<?= $st['id_ptsl']?>"><span class="fa fa-area-chart"></span></button><?php
                          }
                          ?>
												</div>
										 </tr>
    ...
    <div id="modal-luas" class="modal fade" role="dialog">
        ...
        //       async: 		true,
    		...
    		// 			}
    		// 	});
    		// 	$('#form-luas').attr('action','<?=base_url()?>Studio_5_2/simpan_luas/'+idptsl);
    		// 	$('#form-luas')[0].reset();
