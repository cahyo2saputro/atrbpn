								...
								<?php
								 ...
								 <tr>
									...
									<td style="text-align: center;">
								 		<div class="btn-group">
											<?php
													if (in_array(120, $_SESSION['menu']) || $user['level_usr']==1) {
														?><?=$button_analog?><?php
													}
												 ?>
									...
									<td style="text-align: center;"><?=($data->target-$data->jmlluas)?></td>
								 	<td>
								 		<div class="btn-group">
											...
											<?php if((in_array(108, $_SESSION['menu']) || $user['level_usr']==1) && $data->jml_peta > 0){
...
<script type="text/javascript">
	$(document).ready(function () {
		...
		$('#tabel-body').on('click','#btn-cari-kelurahan',function () {
			...
			window.open('<?=base_url()?>studio_5_1/index/?search='+s_filter,'_self',false);
		});

		$('#cari_filter').on('click',function () {
			var s_filter = $('#filter_kecamatan').val();
			var s_filterkelurahan = $('#filter_kelurahan').val();
			window.open('<?=base_url()?>studio5/?filter='+s_filter+'&filterkelurahan='+s_filterkelurahan,'_self',false);
		})
			...
			$('#tabel-body').on('click','#btn-map-online',function () {
				var s_filter = $(this).data('id');
				window.open('<?=base_url()?>studio5/petaonline/'+s_filter,'_self',false);
			});
	})
</script>
