...
class Studio6 extends CI_Controller
{
	var $userdata = NULL;
	...
	{
		$user = $this->auth_model->get_userdata();
		$this->content['data']['title'] = " e-Pemetaan";
		$this->content['data']['subtitle'] = array(array("e-Pemetaan","Studio6"));
		...
		$config['base_url'] = base_url().'Studio6/index/';
		...
		$this->content['studio'] = $this->studio3_model->show_dataluasnib($config['per_page'],$from,$cari,$carikelurahan);

		$this->content['link'] = $this->pagination->create_links();

		$this->content['load'] = array("studio6/data_studio_6");
	...
	function dashboard()
	{
		...
		$this->content['studio'] = $this->studio3_model->get_rank3(50,$from,$cari,$carikelurahan,$tahun);
	...
	public function petaonline($cari){
		...
		$this->content['data']['subtitle'] = array(array("e-Pemetaan","Studio6"),array("Peta Online ".$hasil['nma_kel'],"Studio6/petaonline/".$cari));
