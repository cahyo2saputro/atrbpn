...
class Ajax extends CI_Controller {
    ...
    public function upload_image($id) {
      ...
      if($_FILES["file"]["name"] != '')
      {
          ...
          $ext = end($test);
          $name = "Berkas_".$this->input->post('inputname').''.time().'.' . $ext;
          $location = './DATA/BERKAS/' . $name;  
          move_uploaded_file($_FILES["file"]["tmp_name"], $location);
          // echo $this->input->post('inputname');
          echo "<input type='hidden' name='berkasname[]' value='$name'>";
          echo '<img src="'.base_url().'/DATA/BERKAS/'.$name.'" height="150" width="225" class="img-thumbnail" />';

          if($id=='edit'){
            $ar = array(
              'idptsl_pbk' => $this->input->post('inputname'),
              'berkas_pbk' => $name
            );
    ...
    public function get_anak() {
      ...
      $k4['type'] 	= "multiple";
      $k4['table']  = "tb_anak";
      $k4['join']['table'] = "tb_penduduk";
      $k4['join']['key']   = "idpdk_pdk";
      $k4['join']['ref']   = "idpdk_ank";
      $k4['condition']['noktp_pdk'] = $nik;
      $data = $this->crud_model->get_data($k4);
    ...
    public function cek_data($jenis=null) {
      $id = $this->input->get('id');

      $k4['type'] 	= "single";
      $k4['table'] = "tb_gtra";
      if($jenis=='nib'){
        $k4['condition']['nib_gtra'] = $id;
      }else{
        $k4['condition']['ppptr_gtra'] = $id;
      }
    ...
    public function simpanpenduduk() {
      $simpan = false;
          ...
          $datktp['type'] = "single";
          $datktp['column'] = "count(noktp_pdk) as bpnnik";
          $datktp['like']['noktp_pdk'] = 'BPNNIK-';
          $datktp['orderby']['column'] = 'noktp_pdk';
          $datktp['orderby']['sort'] = 'desc';
          $ktp = $this->crud_model->get_data($datktp);  

          if(!$ktp){
            $noktp='BPNNIK-1';
          }else{
            $noktp='BPNNIK-'.($ktp['bpnnik']+1);
          }
            ...
            'notelp_pdk' => $this->input->post('telp'),
						'idusr_pdk' => $user['idusr_usr']
					);
					$simpan = $this->crud_model->input('tb_penduduk',$ar);

					$insert_id = $this->db->insert_id();
					$this->referensi_model->save_logs($user['idusr_usr'],'tb_penduduk',$insert_id,"Menambahkan Data Penduduk dengan rincian ".displayArray($ar));
      ...
      $berkas['type'] = "multiple";
        ...
        if($fungsi==3){
              ...
              'rt_pdk' => $this->input->post('rt'),
    				...
    				$simpan = $this->crud_model->input('tb_penduduk',$ar);
    				$insert_id = $this->db->insert_id();
    				$this->referensi_model->save_logs($user['idusr_usr'],'tb_penduduk',$insert_id,"Menginput Data Penduduk dengan rincian ".displayArray($ar));
    			}else{
    				$ar = array(
    					'nma_pdk'   => $this->input->post('nama'),
    					'ttl_pdk' => $this->input->post('ttl'),
              ...
              'rt_pdk' => $this->input->post('rt'),
    				...
    				$insert_id = $ktp['idpdk_pdk'];
    				$this->referensi_model->save_logs($user['idusr_usr'],'tb_penduduk',$this->input->post('ktp'),"Mengedit Data Penduduk dengan rincian ".displayArray($ar));
    			}
    	          ...
    	          $_FILES['file']['size'] = $_FILES['berkas']['size'][$i];
    		        ...
    		        $sum = count($file);
    						...
    						if($data1){
    							...
    							$simpan = $this->crud_model->input('tb_ptslberkas',$ar);
    							$this->referensi_model->save_logs($user['idusr_usr'],'tb_ptslberkas',$this->input->post('templateid'),"Add Berkas dengan rincian ".displayArray($ar));
    						}
    				}
          }

    			$this->referensi_model->save_logs($user['idusr_usr'],'tb_ptsl',$this->input->post('templateid'),"Mengedit Data PTSL dengan rincian ".displayArray($dataarray));
          ...
          $this->referensi_model->save_logs($user['idusr_usr'],'tb_ptsl',$this->input->post('templateid'),"Edit Luas PTSL dengan rincian ".displayArray($dataarray));
        }else if($fungsi==6){
          $dataarray = array(
  					'luasfisik_ptsl'   => $this->input->post('luas'),
  					'nib_ptsl'   => $this->input->post('nib'),
          ...
          $this->referensi_model->save_logs($user['idusr_usr'],'tb_ptsl',$this->input->post('templateid'),"Edit Luas dan berkas PTSL dengan rincian ".displayArray($dataarray));
        }else if($fungsi==7){
          $datktp['table'] = "tb_penduduk";
    			$datktp['type'] = "single";
    			$datktp['condition']['noktp_pdk'] = $this->input->post('ktp');
    ...
    public function download_peta() {
      ...
      <input type='hidden' value='analog_pal[]' id='nameform'><input type='hidden' value='<?=$descin?>' id='descform'><?php
      ...
      if(!$sdata){
        ?><div class="form-group row">
          ...
          <div class="col-sm-3">
            <input type="text" class="form-control input-sm" name="desc_pal[]">
          </div>
        </div><?php
      }else{
        foreach ($sdata as $dd) {
          ?><div class="form-group row" id='exs<?=$dd['id_pal']?>'>
            ...
            <div class="col-sm-4">
              <input type='text' class='form-control' id='desc<?=$dd['id_pal']?>' value='<?=$dd['desc_pal']?>'>
            </div>
            <div class="col-sm-1">
              <a class='btn btn-info' data-toggle="tooltip" title='edit deskripsi' onClick='updatedesc(<?=$dd['id_pal']?>)'><span class="fa fa-edit"></span></a>
            </div>
            <div class="col-sm-1">
      ...
      if($sdata){
        foreach ($sdata as $dd) {
          ?><div class="form-group row" id='exs<?=$dd[$primary]?>'>
            <label class="col-sm-3"><?= fdate($dd['create_at'],'HHDDMMYYYY')?> </label>
            <div class="col-sm-1">
              <a class='btn btn-warning' data-toggle="tooltip" title='download berkas' href='<?= base_url().''.$url.''.$dd[$value]?>'><span class="fa fa-cloud-download"></span></a>
            </div>
            <label class="col-sm-6"><i><?=$dd[$desc]?></i></label>
          </div><?php
        }
      }
    }

    public function cari_berkasgtra() {
      $idblk = $this->input->get('idblk');

      $data['type'] 	= "multiple";
      $data['table']  = "tb_gtraberkas";
      $data['condition']['idgtra_pbk'] = $idblk;
      $sdata = $this->crud_model->get_data($data);
      ...
      if($sdata){
        ...
        foreach ($sdata as $dd) {
          ...
          if(strtolower($ext[1])=='pdf'){
            ?><div class="form-group row">
              <label class="col-sm-3">Berkas pdf</label>
              <div class="col-sm-1">
                <a target='_blank' class='btn btn-warning' data-toggle="tooltip" title='download berkas' href='<?= base_url().'DATA/BERKASGTRA/'.$dd['berkas_pbk']?>'><span class="fa fa-cloud-download"></span></a>
              </div>
            </div><?php
          }else{
            $png=1;
        ...
        if($png==1){
          ?>
          <div class="form-group row">
            <label class="col-sm-3">Semua Berkas non-pdf</label>
            <div class="col-sm-1">
              <a target='_blank' class='btn btn-warning' data-toggle="tooltip" title='download berkas' href='<?= base_url().'egtra/exportberkas/'.$idblk?>'><span class="fa fa-cloud-download"></span></a>
            </div>
          </div>
          <?php
        }
      }else{
        echo '<a class="btn btn-warning">Tidak ada berkas yang diupload</a>';
      }
    }
    ...
    public function cari_berkas($jenis) {
      ...
      ?><input type='hidden' value='<?=count($sdata)?>' id='sumfile'>
      <input type='hidden' value='<?=$name?>' id='nameform'><input type='hidden' value='<?=$descin?>' id='descform'><?php
      ...
      if(!$sdata){
        ?><div class="form-group row">
          ...
          <div class="col-sm-3">
            <input type="text" class="form-control input-sm" placeholder="deskripsi" name="<?=$descin?>">
          </div>
        </div><?php
      }else{
        foreach ($sdata as $dd) {
          ?><div class="form-group row" id='exs<?=$dd[$primary]?>'>
            ...
            <div class="col-sm-4">
              <input type='text' class='form-control' id='desc<?=$dd[$primary]?>' value='<?=$dd[$desc]?>'>
            </div>
            <div class="col-sm-1">
              <a class='btn btn-info' data-toggle="tooltip" title='edit deskripsi' onClick='updatedesc(<?=$dd[$primary]?>,"<?=$jenis?>")'><span class="fa fa-edit"></span></a>
            </div>
            <div class="col-sm-1">
    ...
    public function hapus_berkas($jenis) {
      ...
      }else if($jenis=='berkasgtra'){
        $data['type'] 	= "single";
        $data['table']  = "tb_gtraberkas";
        $data['condition']['id_pbk'] = $id;
        $sdata = $this->crud_model->get_data($data);
        $delete = $this->crud_model->delete('tb_gtraberkas',array('id_pbk'=>$id));
        unlink('./DATA/BERKASGTRA/'.$sdata['berkas_pbk']);
      }else if($jenis=='analog'){
        $data['type'] 	= "single";
        $data['table']  = "tb_petaanalog";
    ...
    public function update_berkas($jenis) {

      $id = $this->input->get('id');
      $desc = $this->input->get('desc');

      if($jenis=='gu'){
        $simpan = $this->crud_model->update('tb_blockgu',array('desc_blk'=>$desc),array('id_bgu'=>$id));
      }else if($jenis=='ukur'){
        $simpan = $this->crud_model->update('tb_blockukur',array('desc_blk'=>$desc),array('id_buk'=>$id));
      }else if($jenis=='datmen'){
        $simpan = $this->crud_model->update('tb_blockdatmen',array('desc_blk'=>$desc),array('id_bdm'=>$id));
      }else if($jenis=='ptsl'){
        $simpan = $this->crud_model->update('tb_blockptsl',array('desc_blk'=>$desc),array('id_bpt'=>$id));
      }else if($jenis=='berkas'){

      }else if($jenis=='analog'){
        $simpan = $this->crud_model->update('tb_petaanalog',array('desc_pal'=>$desc),array('id_pal'=>$id));
      }else if($jenis=='sertipikat'){
    ...
    public function cari_dhkp() {
      ...
      <div class='form-group col-sm-12'>
        ...
        <div class='col-sm-6 form-inline'>
          ...
          <input type='text' name='sppt' style='width:40%;float:left' placeholder='no.sppt' class='form-control' id='nosppt' <?=$attr?>  value='<?=$dkel['nosppt_dhkp']?>'>
        </div>
      </div>
      <div class='form-group col-sm-12'>
        <div class='col-sm-2'>
      ...
      <div class='form-group col-sm-12'>
        ...
        <div class='col-sm-6'>
            <input type='text' placeholder='Luas SPPT' id='luassppt' name='luassppt' <?=$attr?> value='<?=$dkel['luassppt_dhkp'];?>' class='form-control'>
        </div>
      </div>
      <div class='form-group col-sm-12'>
        <div class='col-sm-2'>
    ...
    public function cari_dhkp_json() {
      $id = $this->input->get('id');
    ...
    public function simpan_dhkp() {
      $nosppt = $this->input->post('nosspt');
      $block = $this->input->post('block');
      $nwp = $this->input->post('nwp');
      $njop = $this->input->post('njop');
      $luassppt = $this->input->post('luassppt');
      $aop = $this->input->post('aop');
      ...
      $dhkp['join']['key'] = "idblk_dhkp";
      $dhkp['join']['ref'] = "idblk_blk";
      $dhkp['condition']['nosppt_dhkp'] = $nosppt;
      $dhkp['condition']['idblk_dhkp'] = $block;
      $datadhkp = $this->crud_model->get_data($dhkp);

      if($datadhkp){
        $ar = array(
          'nama_dhkp' => $nwp,
          'njopsppt_dhkp' => $njop,
          'luassppt_dhkp' => $luassppt,
          'aopsppt_dhkp' => $aop,
          'awpsppt_dhkp' => $awp,
        );
        $simpan = $this->crud_model->update('tb_dhkp',$ar,array('id_dhkp'=>$datadhkp['id_dhkp']));

        $data['msg'] = 'Data SPPT terdaftar telah diupdate';
        $data['value'] = $datadhkp['id_dhkp'];
        $data['text'] = createkodebpkad($datadhkp['idkel_blk']).''.$datadhkp['nama_blk'].''.$datadhkp['nosppt_dhkp'];
      }else{
        $ar = array(
          'idblk_dhkp' => $block,
          'nosppt_dhkp' => $nosppt,
        ...
        $simpan = $this->crud_model->input('tb_dhkp',$ar);

        $insert_id = $this->db->insert_id();

        $insertdhkp['type'] 	= "single";
        $insertdhkp['table'] = "tb_dhkp";
        $insertdhkp['join']['table'] = "tb_block";
        $insertdhkp['join']['key'] = "idblk_dhkp";
        $insertdhkp['join']['ref'] = "idblk_blk";
        $insertdhkp['condition']['id_dhkp'] = $insert_id;
        $datadhkp = $this->crud_model->get_data($insertdhkp);

        $data['msg'] = 'Data SPPT berhasil didaftarkan';
        $data['value'] = $datadhkp['id_dhkp'];
        $data['text'] = createkodebpkad($datadhkp['idkel_blk']).''.$datadhkp['nama_blk'].''.$datadhkp['nosppt_dhkp'];
    ...
    public function cek_dhkp() {
      ...
      if($datadhkp){
        $data['button'] = 'Update SPPT yang sudah terdaftar';
        $data['data'] = $datadhkp;
      }else{
        $data['button'] = 'Tambahkan data SPPT baru';
        $data['data'] = null;
      }
    ...
    public function getberkas() {
      ...
      if($jenis=='SU'){
        ...
        $data = $this->crud_model->get_data($table);
        $user = $this->auth_model->get_userdata();
        get_berkas('SU',$data,$user['idusr_usr']);
        $file_pdf_kosong = base_url().'digitalisasi/'.$this->session->userdata['view_data']['datax'].'#toolbar=0';
        ?>
        <embed id='wide' style='width:100%;height:600px' src='<?= $file_pdf_kosong; ?>'>
        <embed id='mobile' style='width:100%;height:600px' src='https://docs.google.com/viewer?url=<?= $file_pdf_kosong."&embedded=true"; ?>'>
        <?php
        ...
        $file_pdf_kosong = 'http://api.e-atrbpn.net/digitalisasi/'.strtoupper($data['nma_kec']).'/'.str_replace(' ','_',strtoupper($data['nma_kel'])).'/SURAT_UKUR/SU_'.$data['kd_full'].'_'.$data['no_su'].'_'.$data['thn_su'].'.pdf#toolbar=0';
        ?>
        <embed id='wide' style='width:100%;height:600px' src='<?= $file_pdf_kosong; ?>'>
        <embed id='mobile' style='width:100%;height:600px' src='https://docs.google.com/viewer?url=<?= $file_pdf_kosong."&embedded=true"; ?>'>
        <?php
        ...
        $file_pdf_kosong = 'https://api.e-atrbpn.net/digitalisasi/'.strtoupper($data['nma_kec']).'/'.str_replace(' ','_',strtoupper($data['nma_kel'])).'/SURAT_UKUR/GS_'.$data['kd_full'].'_'.$data['no_su'].'_'.$data['thn_su'].'.pdf#toolbar=0';
        ?>
        <embed id='wide' style='width:100%;height:600px' src='<?= $file_pdf_kosong; ?>'>
        <embed id='mobile' style='width:100%;height:600px' src='<?= $file_pdf_kosong; ?>'>
        <?php
        ...
        $table['column'] 	= "nma_kel,nma_kec,kd_full,REPLACE(tb_hak.no_hak,'.','') as nohakfile";
        $table['condition']['no_hak']= $nohak;
        $data = $this->crud_model->get_data($table);
        $bt = str_replace('.','',$nohak);

        $file_pdf_kosong = 'http://api.e-atrbpn.net/digitalisasi/'.strtoupper($data['nma_kec']).'/'.str_replace(' ','_',strtoupper($data['nma_kel'])).'/BUKU_TANAH/BT_'.$data['nohakfile'].'.pdf#toolbar=0';
        ?>
        <embed id='wide' style='width:100%;height:600px' src='<?= $file_pdf_kosong; ?>'>
        <embed id='mobile' style='width:100%;height:600px' src='https://docs.google.com/viewer?url=<?= $file_pdf_kosong."&embedded=true"; ?>'>
        <?php
        ...
        $bt = str_replace('.','',$nohak);
        $user = $this->auth_model->get_userdata();
        get_berkas('BT',$data,$user['idusr_usr']);
        $file_pdf_kosong = base_url().'digitalisasi/'.$this->session->userdata['view_data']['datax'].'#toolbar=0';
        ?>
        <embed id='wide' style='width:100%;height:600px' src='<?= $file_pdf_kosong; ?>'>
        <embed id='mobile' style='width:100%;height:600px' src='https://docs.google.com/viewer?url=<?= $file_pdf_kosong."&embedded=true"; ?>'>
    ...
    public function uploadimage() {
        ...
        if($this->input->post('id')){
          ...
          $ar = array(
            'screenshoot' => $name,
            'idusr_scr' => $user['idusr_usr']
          );
          $simpan = $this->crud_model->update('tb_keluhan',$ar,array('id'=>$id));
        }else{
          $ar = array(
            'screenshoot' => $name,
            'idusr_scr' => $user['idusr_usr'],
          ...
          $simpan = $this->crud_model->input('tb_keluhan',$ar);
          $id = $this->db->insert_id();
        }
        
        $response = array('filename' => $name,'id' => $id);
